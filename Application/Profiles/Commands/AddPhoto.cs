...
public class AddPhoto
{
    ...
    public class Handler(IUserAccessor userAccessor, AppDbContext context, IPhotoService photoService) : IRequestHandler<Command, Result<Photo>>
    {
        public async Task<Result<Photo>> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            user.ImageUrl ??= photo.Url; // if the user doesn't have a main photo, set this as main

            // add the photo to the user's collection and save changes
            context.Photos.Add(photo);
            var result = await context.SaveChangesAsync() > 0;

            return result 
                ? Result<Photo>.Success(photo) 
                : Result<Photo>.Failure("Problem adding photo to database", 400);
        }
    }
